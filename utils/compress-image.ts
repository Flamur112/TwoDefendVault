...
export function outputNameForMime(file: File, mime: string): string {
...
export async function compressImageFile(
  ...
  const maxOutputBytes = options.maxOutputBytes ?? IMAGE_MAX_OUTPUT_BYTES
  ...
  if (keepPng) {
    ...
    if (pngBlob.size <= maxOutputBytes) {
  ...
  if (blob.size > maxOutputBytes) {
    throw new Error(
      `Image is still too large after compression (${formatFileSize(blob.size)}). Try a smaller or simpler image.`,
    )
  }
